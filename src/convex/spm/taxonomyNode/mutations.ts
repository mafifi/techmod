...
export const create = mutation({
	...
	args: zodToConvex(
		TaxonomyNodePropsSchema.omit({ version: true, lastModified: true, changeHistory: true })
	),
...
export const createPortfolio = mutation({
	...
	args: zodToConvex(
		PortfolioPropsSchema.omit({ version: true, lastModified: true, changeHistory: true })
	),
	handler: async (ctx: MutationCtx, args: any) => {
...
export const createLine = mutation({
	args: zodToConvex(
		LinePropsSchema.omit({ version: true, lastModified: true, changeHistory: true })
	),
	handler: async (ctx: MutationCtx, args: any) => {
...
export const createCategory = mutation({
	args: zodToConvex(
		CategoryPropsSchema.omit({ version: true, lastModified: true, changeHistory: true })
	),
	handler: async (ctx: MutationCtx, args: any) => {
...
export const update = mutation({
	args: {
		...
		updates: zodToConvex(
			TaxonomyNodePropsSchema.partial().omit({
				createdBy: true,
				changeHistory: true,
				version: true
			})
		...
		if (updates.parentId !== undefined) {
			if (updates.parentId !== existing.parentId) {
				...
				const hasCircular = await checkCircularReference(
					ctx,
					nodeId as Id<'taxonomyNodes'>,
					updates.parentId as Id<'taxonomyNodes'> | null
				);
				if (hasCircular) {
...
export const deleteNode = mutation({
	...
	handler: async (ctx: MutationCtx, args: any) => {
		...
		if (!forceDelete) {
			...
			if (children) {
				throw new Error(
					'Cannot delete node with children. Use forceDelete or move children first.'
				);
			}
		} else {
