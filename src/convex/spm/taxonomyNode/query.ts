...
export const getPortfolios = query({
	...
	handler: async (ctx: QueryCtx, args: { activeOnly?: boolean }) => {
		...
		let q = ctx.db
			.query('taxonomyNodes')
			.withIndex('by_type', (q) => q.eq('type', 'portfolio'));
...
export const getLines = query({
	...
	handler: async (ctx: QueryCtx, args: { portfolioId?: Id<'taxonomyNodes'>, activeOnly?: boolean }) => {
		const { portfolioId, activeOnly = true } = args;
		
		let q = ctx.db
			.query('taxonomyNodes')
			.withIndex('by_type', (q) => q.eq('type', 'line'));
...
export const getChildren = query({
	...
	handler: async (ctx: QueryCtx, args: { parentId: Id<'taxonomyNodes'>, type?: 'portfolio' | 'line' | 'category', activeOnly?: boolean }) => {
...
export const search = query({
	...
	handler: async (ctx: QueryCtx, args: { query: string, type?: TaxonomyNodeType, activeOnly?: boolean }) => {
		...
		return results.filter((node: any) =>
			searchRegex.test(node.name) ||
			(node.description && searchRegex.test(node.description))
		);
	}
});
...
export const validateParentChild = query({
	...
	handler: async (ctx, args) => {
		...
		if (childType === 'portfolio') {
			if (parentId !== null) {
				return { valid: false, reason: 'Portfolios cannot have parent nodes - they must be top-level' };
			}
			return { valid: true };
		}
