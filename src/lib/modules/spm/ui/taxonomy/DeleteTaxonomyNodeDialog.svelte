		...
		<div class="grid gap-4 py-4">
			...
			<div class="bg-muted/50 p-4 rounded-lg border">
				<div class="flex items-start gap-3">
					...
					<div class="min-w-0 flex-1">
						<h3 class="font-medium text-sm">{node.name}</h3>
			...
			{#if hasChildren}
				<Alert variant="destructive">
					...
					<AlertDescription>
						<strong>Warning:</strong> This {info.label.toLowerCase()} contains {childCount} {info.childLabel}.
						Deleting it will also delete all its child nodes.
					</AlertDescription>
				</Alert>
				...
				<div class="space-y-3">
					<div class="flex items-center space-x-2">
						...
						<Label for="force-delete" class="text-sm leading-relaxed">
							I understand that deleting this {info.label.toLowerCase()} will permanently remove 
							all {childCount} {info.childLabel} and any data associated with them.
						</Label>
					</div>
				</div>
			{:else}
				<Alert>
					<AlertTriangle class="h-4 w-4" />
					<AlertDescription>
						This {info.label.toLowerCase()} will be permanently deleted. 
			...
			<div class="text-xs text-muted-foreground space-y-2">
				...
				<p>
					...
					<strong>Status:</strong> {node.isActive ? 'Active' : 'Inactive'}
				</p>
			</div>
		</div>
