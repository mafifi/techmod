...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '$lib/ui/components/dialog';
...
let { mode, node, nodeType, parentId = null, validParentOptions, onSubmit, onCancel }: Props = $props();
...
let selectIsActive: any = formData.isActive ? 'true' : 'false';

$effect(() => {
	// keep the string select synced with the boolean formData
	selectIsActive = formData.isActive ? 'true' : 'false';
});

$effect(() => {
	// when select changes, update boolean formData
	formData.isActive = selectIsActive === 'true';
});
...
function getParentDisplayName(parent: TaxonomyHierarchyNode): string {
	const parentInfo = typeInfo[parent.type];
	return `${parent.name} (${parentInfo.label})`;
}

// Precompute parent type label to avoid indexing issues in template (runes-compatible)
const parentTypeName = $derived((info as any).parentType ? (typeInfo as any)[(info as any).parentType].label : '');
</script>
			...
			<div class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					...
					<div class="space-y-2">
						...
						<Select type="single" bind:value={selectIsActive}>
				...
				<!-- Strategy (optional) -->
				<div class="space-y-2">
					<Label for="strategy">Strategy <span class="text-muted-foreground">(optional)</span></Label>
					<Textarea
						id="strategy"
					...
					<div class="space-y-2">
						...
						{#if validParentOptions.length === 0}
							<Alert>
								...
								<AlertDescription>
									No valid parent {parentTypeName.toLowerCase()}s available. 
									You need to create a {parentTypeName.toLowerCase()} first.
								</AlertDescription>
							</Alert>
						{:else}
							<Select type="single" bind:value={formData.parentId}>
								<SelectTrigger id="parent" class={errors.parentId ? 'border-red-500' : ''}>
									{formData.parentId ? 
										validParentOptions.find(p => p._id === formData.parentId)?.name || 'Select parent' :
										`Select parent ${parentTypeName.toLowerCase()}`}
								</SelectTrigger>
								<SelectContent>
									{#each validParentOptions as parent}
